...
mixin _$Shift {
 ...
 String get id; String get userId; DateTime get startedAt; ShiftStatus get status; List<Pause> get pauses; int get workedSeconds; String? get organizationId;/// IANA-таймзона организации на момент ответа (`Organization.timezone`,
...
/// Никогда не подставляется дефолтная зона на этом уровне.
 String? get organizationTimezone; DateTime? get finishedAt; bool get hasIncompleteRequiredChecklists;/// Рабочая точка, на которой открыта смена. `null` — не определена
...
/// исторических смен, заведённых до фичи графиков.
 ...
 ShiftOvertimeRequest? get overtime;/// Смена заведена админом вручную (`created_by_user_id IS NOT NULL`), а
/// не начата сотрудником (`manual_time_entry`). `false` — старый бэк или
/// обычная смена.
 bool get isManual;/// Смену когда-либо правил админ (`edited_at IS NOT NULL`).
 bool get isEdited;/// Комментарий/причина последней ручной операции (создания или правки).
/// `null` — не задан.
 String? get manualNote;/// Момент последней ручной правки. `null` — не правилась.
 DateTime? get editedAt;/// Имя админа, создавшего смену вручную. Заполнено только в
/// орг-ответах; в персональном контексте (`GET /shifts`) — всегда `null`
/// (сотруднику достаточно факта правки и [manualNote]).
 String? get createdByName;/// Имя админа, последним правившего смену. Заполнено только в
/// орг-ответах; в персональном — всегда `null`.
 String? get editedByName;/// Смена удалена (soft-delete). В обычных выборках всегда `false`.
 bool get isDeleted;/// Смена стартовала без геопроверки — по фото вместо координат
/// (`shift_geo_photo_fallback`). `false` — обычная смена либо старый бэк.
/// В UI сотрудника не выводится (вне scope) — признак нужен админке.
 bool get geoFallback;/// Машинный код гео-ошибки, приведшей в fallback (`GEO_*` из таксономии
/// `GeoService`). `null` — обычная смена.
 String? get geoFallbackReason;/// Файл фото, приложенного к fallback-старту. `null` — обычная смена или
/// файл удалён (FK `ON DELETE SET NULL`, причина при этом остаётся).
 String? get geoFallbackPhotoFileId;/// Заработок по этой смене (`shift_history_earnings/backend.md`, п.1,
/// additive). `null` — смена персональная либо не в статусе `finished`
/// (ADR-005 п.6/8), см. [ShiftEarnings].
 ShiftEarnings? get earnings;
...
as ShiftOvertimeRequest?,isManual: null == isManual ? _self.isManual : isManual // ignore: cast_nullable_to_non_nullable
...
@override
@pragma('vm:prefer-inline')
$ShiftEarningsCopyWith<$Res>? get earnings {
    if (_self.earnings == null) {
    return null;
  }

  return $ShiftEarningsCopyWith<$Res>(_self.earnings!, (value) {
...
@optionalTypeArgs TResult maybeWhen<TResult extends Object?>(TResult Function( String id,  String userId,  DateTime startedAt,  ShiftStatus status,  List<Pause> pauses,  int workedSeconds,  String? organizationId,  String? organizationTimezone,  DateTime? finishedAt,  bool hasIncompleteRequiredChecklists,  String? workLocationId,  WorkLocation? workLocation,  String? userName,  String? userEmail,  String? role,  String? customRoleName,  String? workScheduleId,  String? scheduleName,  DateTime? scheduledStartAt,  DateTime? scheduledEndAt,  int? lateSeconds,  ShiftFinishReason? finishReason,  ShiftOvertimeRequest? overtime,  bool isManual,  bool isEdited,  String? manualNote,  DateTime? editedAt,  String? createdByName,  String? editedByName,  bool isDeleted,  bool geoFallback,  String? geoFallbackReason,  String? geoFallbackPhotoFileId,  ShiftEarnings? earnings)?  $default,{required TResult orElse(),}) {final _that = this;
switch (_that) {
case _Shift() when $default != null:
...
@optionalTypeArgs TResult when<TResult extends Object?>(TResult Function( String id,  String userId,  DateTime startedAt,  ShiftStatus status,  List<Pause> pauses,  int workedSeconds,  String? organizationId,  String? organizationTimezone,  DateTime? finishedAt,  bool hasIncompleteRequiredChecklists,  String? workLocationId,  WorkLocation? workLocation,  String? userName,  String? userEmail,  String? role,  String? customRoleName,  String? workScheduleId,  String? scheduleName,  DateTime? scheduledStartAt,  DateTime? scheduledEndAt,  int? lateSeconds,  ShiftFinishReason? finishReason,  ShiftOvertimeRequest? overtime,  bool isManual,  bool isEdited,  String? manualNote,  DateTime? editedAt,  String? createdByName,  String? editedByName,  bool isDeleted,  bool geoFallback,  String? geoFallbackReason,  String? geoFallbackPhotoFileId,  ShiftEarnings? earnings)  $default,) {final _that = this;
switch (_that) {
case _Shift():
return $default(_that.id,_that.userId,_that.startedAt,_that.status,_that.pauses,_that.workedSeconds,_that.organizationId,_that.organizationTimezone,_that.finishedAt,_that.hasIncompleteRequiredChecklists,_that.workLocationId,_that.workLocation,_that.userName,_that.userEmail,_that.role,_that.customRoleName,_that.workScheduleId,_that.scheduleName,_that.scheduledStartAt,_that.scheduledEndAt,_that.lateSeconds,_that.finishReason,_that.overtime,_that.isManual,_that.isEdited,_that.manualNote,_that.editedAt,_that.createdByName,_that.editedByName,_that.isDeleted,_that.geoFallback,_that.geoFallbackReason,_that.geoFallbackPhotoFileId,_that.earnings);case _:
  throw StateError('Unexpected subclass');
...
@optionalTypeArgs TResult? whenOrNull<TResult extends Object?>(TResult? Function( String id,  String userId,  DateTime startedAt,  ShiftStatus status,  List<Pause> pauses,  int workedSeconds,  String? organizationId,  String? organizationTimezone,  DateTime? finishedAt,  bool hasIncompleteRequiredChecklists,  String? workLocationId,  WorkLocation? workLocation,  String? userName,  String? userEmail,  String? role,  String? customRoleName,  String? workScheduleId,  String? scheduleName,  DateTime? scheduledStartAt,  DateTime? scheduledEndAt,  int? lateSeconds,  ShiftFinishReason? finishReason,  ShiftOvertimeRequest? overtime,  bool isManual,  bool isEdited,  String? manualNote,  DateTime? editedAt,  String? createdByName,  String? editedByName,  bool isDeleted,  bool geoFallback,  String? geoFallbackReason,  String? geoFallbackPhotoFileId,  ShiftEarnings? earnings)?  $default,) {final _that = this;
switch (_that) {
case _Shift() when $default != null:
return $default(_that.id,_that.userId,_that.startedAt,_that.status,_that.pauses,_that.workedSeconds,_that.organizationId,_that.organizationTimezone,_that.finishedAt,_that.hasIncompleteRequiredChecklists,_that.workLocationId,_that.workLocation,_that.userName,_that.userEmail,_that.role,_that.customRoleName,_that.workScheduleId,_that.scheduleName,_that.scheduledStartAt,_that.scheduledEndAt,_that.lateSeconds,_that.finishReason,_that.overtime,_that.isManual,_that.isEdited,_that.manualNote,_that.editedAt,_that.createdByName,_that.editedByName,_that.isDeleted,_that.geoFallback,_that.geoFallbackReason,_that.geoFallbackPhotoFileId,_that.earnings);case _:
  return null;
...
/// additive). `null` — смена персональная либо не в статусе `finished`
/// (ADR-005 п.6/8), см. [ShiftEarnings].
@override final  ShiftEarnings? earnings;
...
@override $WorkLocationCopyWith<$Res>? get workLocation;@override $ShiftOvertimeRequestCopyWith<$Res>? get overtime;@override $ShiftEarningsCopyWith<$Res>? get earnings;
