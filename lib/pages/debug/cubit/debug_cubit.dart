...
class DebugCubit extends Cubit<DebugState> {
  DebugCubit({required DebugRepository debugRepository})
  ...
  Future<void> _init() async {
    await Future.wait([fetchFlavor(), fetchDeviceAccess()]);
  }
  ...
  Future<void> fetchFlavor() async {
    ...
    try {
      ...
      emit(state.copyWith(flavor: flavor, status: FeatureStatus.success));
    } on Exception catch (e) {
      emit(
        state.copyWith(errorMessage: e.toString(), status: FeatureStatus.error),
      );
    }
  }
  ...
  Future<void> setDeviceAccess() async {
    ...
    emit(state.copyWith(isDeviceAccess: true, status: FeatureStatus.success));
  }
}
