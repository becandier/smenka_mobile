...
- **Geo**: geo-check решает, показывать ли picker, по числу стартуемых графиков и переиспользует уже полученные координаты; geo-fallback применяет ту же выборку после выбора точки и перед отправкой фото-старта. Кубиты остаются независимыми.
