...
## Фаза 3 — Трекер смены (главный экран) `[ ]`
- [ ] **ShiftTracker** — основной экран (Tab 1)
- [ ] Состояние "нет активной смены": кнопка "Начать", выбор персональная/организация
- [ ] Состояние "смена активна": таймер, статус, кнопки пауза/продолжить/завершить
- [ ] Список пауз текущей смены
- [ ] Выбор организации перед стартом (если юзер состоит в орг-ях)
- [ ] Геопроверка: запрос локации, проверка что в радиусе (если орг с geo_check)
- [ ] Кубит: ShiftTrackerCubit
- [ ] Интеграция с ShiftRepository (start, pause, resume, finish)
- [ ] Интеграция с OrganizationRepository (getAll — для выбора орг)
- [ ] Локализация

---

## Фаза 4 — История смен + статистика `[ ]`
- [ ] **ShiftHistory** — список смен с пагинацией (Tab 2)
- [ ] Фильтры: статус (active/paused/finished), дата от-до
- [ ] Карточка смены: дата, длительность, статус, организация
- [ ] **ShiftDetail** — подробности смены (push-страница)
- [ ] **ShiftStats** — статистика: день/неделя/месяц (сегмент на экране истории)
- [ ] Кубиты: ShiftHistoryCubit, ShiftDetailCubit, ShiftStatsCubit
- [ ] Интеграция с ShiftRepository (getShifts, getStats)
- [ ] Локализация

---

## Фаза 5 — Профиль `[ ]`
- [ ] **Profile** — данные пользователя (Tab 3)
- [ ] Отображение: имя, email, телефон, дата регистрации
- [ ] Редактирование профиля (name, phone) — модалка или отдельный экран
- [ ] Переключатель темы (уже есть виджет, нужно встроить)
- [ ] Секция "Мои организации" — список с переходом
- [ ] Кнопка "Выйти" (logout)
- [ ] Кубит: ProfileCubit
- [ ] Интеграция с UserRepository (getMe, updateMe), AuthRepository (logout)
- [ ] Локализация
