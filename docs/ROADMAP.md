...
- [x] `make check` зелёный: analyze без замечаний, 455/455 тестов; новые регрессии — обычный старт, geo-check, geo-fallback и открытие окна
...
## Фича — Единое отображение времени смены в таймзоне организации (shift_timezone_display) `[~]` (`../docs/tasks/shift_timezone_display/mobile.md`)
- [x] **Прод-баг**: `DateTime.parse` UTC-момента шёл прямо в `DateFormat`/`.toLocal()` — организационная смена `04:39:06Z–18:00:00Z` (`Europe/Moscow`) показывалась как `04:39–18:00` вместо `07:39–21:00`; персональная смена на устройстве в другой зоне тоже путала момент с настенным временем
- [x] **Единый time-сервис** `lib/core/time/app_time.dart`: sealed `AppTimeContext.organization(timeZone)` / `.device()` (value equality, `@immutable`) + `AppTime` (`wallTime`/`formatDate`/`formatTime`/`formatDateTime`/`utcBoundsForDay`) — единственная точка форматирования; `.toLocal()` вызывается только внутри `DeviceTimeContext`-ветки. Строится поверх уже существующего `core/utils/org_timezone.dart` (`toOrgLocal`/`orgUtcDayBounds`/`orgLocalDayDiff`, IANA `timezone` package) — не дублирует движок DST
- [x] **Контракт**: `Shift.organizationTimezone`/`ShiftDto.organizationTimezone` — additive nullable (backend `553a235`), `ChecklistInstanceDetail(Dto).organizationTimezone` — additive nullable self-contained detail (backend `d84750c`); маппер не подставляет дефолт вместо `null`. `ShiftTimeContext.timeContext({scopedOrganizationTimezone})`/`ChecklistTimeContext.timeContext()` — единая точка выбора контекста по смене/чек-листу
- [x] **Миграция поверхностей** (аудит — см. `task-3-report.md`): история/детали своей и чужой (орг) смены, паузы, план, время правки, org_shifts список, чек-листы/фото (`captured_at`, антифрод-штамп), штрафы (карточка + форма назначения + shift-picker), корректировки, ставки участника, payroll/my-earnings/org-stats заголовки и календарные фильтры, дедлайны тестов сотрудников, идле-подсказки графика. Платформенные поверхности (уведомления, баннер подписки/tariffs) переведены на тот же сервис с явным `AppTimeContext.device()`, а не оставлены на разрозненном форматировании
- [x] **Календарные фильтры**: org-scoped (org_stats/payroll/my-earnings/my-penalties/my-adjustments/org_shifts, история смен на экране одной организации) строят сутки через `AppTime.utcBoundsForDay` в org-контексте — IANA-сутки 23/25 часов в дни DST, не `add(Duration(days: 1))`; смешанное «Все смены»/персональный контекст — устройство (design.md: один фильтр не может представлять разные календарные сутки нескольких организаций)
- [x] **Архитектурный guard** `test/architecture/time_boundary_test.dart`: сканирует рукописный `lib/**/*.dart` (кроме `.g.dart`/`.freezed.dart`/`.gr.dart`, генерируемой `lib/l10n/app_localizations*.dart` и трёх задокументированных исключений — `app_time.dart`, `org_timezone.dart`, `date_range_picker_page.dart`) на прямой `import 'package:intl/intl.dart'`/`.toLocal()`/`DateFormat(`; проверен внедрённой нарушающей фикстурой (поймал все 3 паттерна, файл удалён)
- [x] **TDD**: `test/core/time/app_time_test.dart` (Москва/Berlin зима-лето/23-25ч DST-сутки/невалидная зона/device — 17 тестов, зелёные под `TZ=UTC|Europe/Moscow|America/Los_Angeles|Pacific/Kiritimati` без расхождений), `test/data/shift/shift_organization_timezone_mapper_test.dart` (rolling-deploy совместимость DTO)
- [x] Никаких non-null assertion (`!`) в рукописном diff (проверено построчно)
