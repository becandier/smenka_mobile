...
- [ ] **End-to-end**: код и клиентская конфигурация готовы, но полный e2e не доступен до внешних шагов — реальные `REVERSED_CLIENT_ID`/`GIDClientID`/Apple-провижининг из консолей, уточнение в `admin.md` про Web Client ID для `(google, android)` (детали — `../docs/tasks/oauth_login/STATUS.md`, «Открытые вопросы к аналитику»)
